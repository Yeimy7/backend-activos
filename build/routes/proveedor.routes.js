...
router.post('/', [check('razon_social', 'La razón social del proveedor es obligatorio').not().isEmpty(), check('encargado', 'El nombre del encargado es obligatorio').not().isEmpty(), check('telefono', 'Agrega un numero de telefono válido').isNumeric().optional({
  nullable: true
}), authJwt.verifyToken, authJwt.isSuperAdminOrAdmin], proveedorController.crearProveedor);
