...
const router = Router();
import * as ambienteController from '../controllers/ambiente.controller.js'; // import { authJwt } from '../middlewares'

import * as authJwt from '../middlewares/authJwt.js';
import { check } from 'express-validator';
router.post('/', [check('codigo_ambiente', 'El código del ambiente es obligatorio').not().isEmpty(), check('tipo_ambiente', 'El tipo de ambiente es obligatorio').not().isEmpty(), check('id_piso', 'El id del piso es obligatorio').not().isEmpty(), authJwt.verifyToken, authJwt.isSuperAdminOrAdmin], ambienteController.crearAmbiente);
