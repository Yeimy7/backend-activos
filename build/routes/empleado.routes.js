...
const router = Router();
import * as empleadoController from '../controllers/empleado.controller.js'; // import { authJwt } from '../middlewares'

import * as authJwt from '../middlewares/authJwt.js';
import { check } from 'express-validator';
router.post('/', [check('nombres', 'El nombre es obligatorio').not().isEmpty(), check('apellidos', 'El apellido es obligatorio').not().isEmpty(), check('ci', 'El ci es obligatorio').not().isEmpty(), check('descripcion_cargo', 'El cargo es obligatorio').not().isEmpty(), authJwt.verifyToken, authJwt.isSuperAdminOrAdmin], empleadoController.crearEmpleado);
