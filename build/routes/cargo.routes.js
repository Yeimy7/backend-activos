...
const router = Router();
import * as cargoController from '../controllers/cargo.controller.js'; // import { authJwt } from '../middlewares'

import * as authJwt from '../middlewares/authJwt.js';
import { check } from 'express-validator';
router.post('/', [check('descripcion_cargo', 'La descripcion del cargo es obligatorio').not().isEmpty(), check('nombre_area', 'El nombre del area es obligatorio').not().isEmpty(), authJwt.verifyToken, authJwt.isSuperAdminOrAdmin], cargoController.crearCargo);
