...
export const crearHdepreciacion = async (req, res) => {
  ...
  try {
    ...
    const valor_ufv = await ValorUfv.findOne({
      raw: true,
      where: {
        id_valor_ufv: id_valor_ufv
      }
    });
    if (!valor_ufv) return res.status(404).json({
      msg: 'Valor UFV no encontrado',
      type: 'error'
    }); //Guardar el datos de depreciacion y actualizar el activo

    const resultado = await Hdepreciacion.create({
      valor_residual,
      id_activo,
      id_valor_ufv
    });
...
export const obtenerHdepreciacionPorId = async (req, res) => {
  try {
    const hdepreciacion = await Hdepreciacion.findOne({
      raw: true,
      where: {
        id_hdepreciacion: req.params.hdepreciacionId
      },
      include: [{
        model: Activo,
        attributes: ['descripcion_activo', 'codigo_activo']
...
export const crearHdepreciaciones = async (req, res) => {
  ...
  try {
    ...
    const activos = await Activo.findAll({
      ...
      where: {
        estado: 'A',
        [Op.and]: [Sequelize.where(Sequelize.fn('YEAR', Sequelize.col('fecha_ingreso')), gestion + 1)]
      }
    });
    await Promise.all(hdepreciaciones.map(async hdepreciacion => {
      const {
        id_activo,
        valor_residual,
        ['activo.indice_ufv']: indice_ufv
      } = hdepreciacion;
      const ajusteActualizacion = valor_residual / indice_ufv * valor_ufv - valor_residual;
      await Hdepreciacion.create({
        valor_residual: ajusteActualizacion + valor_residual,
        id_activo,
        id_valor_ufv: valorUfv[0].id_valor_ufv
      });
      return;
    }));
    await Promise.all(activos.map(async activo => {
      ...
      await Hdepreciacion.create({
        valor_residual: ajusteActualizacion + costo,
        id_activo,
        id_valor_ufv: valorUfv[0].id_valor_ufv
      });
      return;
    ...
    res.status(200).json({
      msg: 'Depreciaciones realizadas exitosamente',
      type: 'success'
    });
  } catch (error) {
    res.status(500).json({
  ...
  try {
    ...
    const itemsActivosNuevos = await Promise.all(activos?.map(async activo => {
      ...
      const {
        ...
        K,
        L
      } = valores; // td = td + Number(activo.costo)
      // te = te + Number(activo.valor_residual)
      // tf = tf + Number(F)
      // tg = tg + Number(G)
      // th = th + Number(H)
    ...
    if (isPdf) {
      ...
      ws.column(19).setWidth(12); //NOmbre de las columnas
      ...
      ws.cell(pos, 19).number(valores.tl).style(contenidoEstilo); //Ruta del archivo

      const __filename = fileURLToPath(import.meta.url);

      const __dirname = path.dirname(__filename);

      const pathExcel = path.join(__dirname, 'excel', nombreArchivo + '.xlsx'); // const pathExcel = path.join(__dirname, 'excel', nombreArchivo + '.xlsx')
      //Escribir o guardar

      wb.write(pathExcel, async (err, stats) => {
        if (err) {
          res.status(500).json({
            msg: 'Error al crear el archivo Excel',
            type: 'error'
          });
        } else {
          ...
          res.sendFile(pathExcel, error => {
            if (error) {
              res.status(500).json({
                msg: 'Error al enviar el archivo Excel al cliente',
                type: 'error'
              });
