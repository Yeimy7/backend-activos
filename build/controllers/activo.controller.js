...
export const crearActivo = async (req, res) => {
  ...
  try {
    ...
    if (!ambiente) return res.status(404).json({
    ...
    if (!auxiliar) return res.status(404).json({
      msg: 'Auxiliar no encontrado',
      type: 'error'
    });
    const grupo = await GrupoContable.findOne({
      where: {
        descripcion_g: descripcion_g
      }
    });
    if (!grupo) return res.status(404).json({
      msg: 'Grupo contable no encontrado',
      type: 'error'
    });
    ...
    if (!proveedor) return res.status(404).json({
...
export const obtenerActivoPorCodigo = async (req, res) => {
  try {
    let activo = await Activo.findOne({
      raw: true,
      where: {
        codigo_activo: req.params.codigoActivo
      },
      attributes: ['codigo_activo', 'fecha_ingreso', 'descripcion_activo', 'img_activo', 'estado'],
      include: [{
        model: Ambiente,
        attributes: ['codigo_ambiente', 'tipo_ambiente'],
        include: [{
...
export const actualizarActivoPorId = async (req, res) => {
  ...
  try {
    ...
    let isModified = false; // Si el proyecto existe o no
...
export const listaActivosExcel = async (_req, res) => {
  ...
  try {
    ...
    ws.column(6).setWidth(25); //NOmbre de las columnas
    ...
    const pathExcel = path.join(__dirname, 'excel', nombreArchivo + '.xlsx'); //Escribir o guardar
...
export const actaAsignacionActivo = async (req, res) => {
  ...
  try {
    ...
    const persona = await Person.findOne({
      raw: true,
      where: {
        id_persona: activo['empleado.id_persona']
      }
    });
...
export const activosPorGrupo = async (req, res) => {
  try {
    const grupoContable = await GrupoContable.findOne({
      raw: true,
      where: {
...
export const totalAsignados = async (_req, res) => {
  try {
    ...
    res.status(500).json({
      msg: 'No se pudo encontrar el total',
      type: 'error'
    });
  }
};
...
export const reporteActivosPorEntidad = async (req, res) => {
  try {
    ...
    const allDataActivos = await Promise.all(activos.map(async activo => {
      ...
      return await {
        ambiente: activo["ambiente.tipo_ambiente"] + " " + activo["ambiente.codigo_ambiente"],
        codigo_activo: activo.codigo_activo,
        descripcion_activo: activo.descripcion_activo,
        grupo_contable: activo["grupo_contable.descripcion_g"],
