...
export const ascendUser = async (req, res) => {
  ...
  try {
    ...
    user.id_rol = rol.id_rol; // newUser.id_rol = rol.id_rol
    // console.log(user)

    await user.save();
    res.status(201).json({
      msg: 'Rol modificado a Administrador',
      type: 'success'
    });
  } catch (error) {
    res.status(500).json({
      msg: 'Hubo un error al recuperar datos de los usuarios'
    });
  }
};
