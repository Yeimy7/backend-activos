...
export const profile = async (req, res) => {
  try {
    const persona = await Person.findAll({
      where: {
        id_persona: req.userId
      }
    });
    ...
    res.status(500).json({
      msg: 'Hubo un error al buscar su perfil',
      type: 'error'
    });
  }
};
...
export const forgotPassword = async (req, res) => {
  ...
  try {
    await transporter.sendMail({
      from: '"Olvide la contraseña  👻" <brielalica@gmail.com>',
      // sender address
      to: usuario.email,
      // list of receivers
      subject: "Olvide la contraseña",
      // Subject line
      html: template
    });
  } catch (error) {
