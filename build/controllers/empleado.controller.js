...
export const crearEmpleado = async (req, res) => {
  ...
  try {
    ...
    const personaRegistrada = await Person.findOne({
      raw: true,
      where: {
        ci: ci
      }
    ...
    if (personaRegistrada) {
      ...
      const registeredPerson = await Person.create({
        ci,
        nombres,
        apellidos
      });
    ...
    res.status(201).json({
      ...
      "cargo.area.nombre_area": cargo['area.nombre_area']
    });
  } catch (error) {
    res.status(500).json({
      msg: 'El número de ci ya esta registrado',
      type: 'error'
    });
  }
};
...
export const obtenerEmpleadoPorId = async (req, res) => {
  try {
    const persona = await Person.findOne({
      raw: true,
      where: {
        id_persona: req.params.empleadoId
      }
    });
    ...
    const datosEmpleado = await { ...persona,
      ...empleado
    };
...
export const actualizarEmpleadoPorId = async (req, res) => {
  ...
  try {
    ...
    if (isModified) {
      ...
      empleadoActualizado = await Empleado.findOne({
        raw: true,
        where: {
