...
export const verifyToken = async (req, res, next) => {
  try {
    ...
    if (!token) return res.status(403).json({
      msg: 'No se proporciona ningún token',
      type: 'unseen'
    });
...
export const isAdmin = async (req, res, next) => {
  try {
    ...
    return res.status(403).json({
      msg: 'Requiere el rol de Administrador',
      type: 'error'
    });
  } catch (error) {
    res.status(401).json({
...
export const isSuperAdmin = async (req, res, next) => {
  try {
    ...
    return res.status(403).json({
      msg: 'Requiere el rol de Super-admin',
      type: 'error'
    });
  } catch (error) {
    res.status(401).json({
...
export const isSuperAdminOrAdmin = async (req, res, next) => {
  try {
    ...
    return res.status(403).json({
      msg: 'Requiere el rol de Super-admin o Administrador',
      type: 'error'
    });
  } catch (error) {
    res.status(401).json({
