...
const router = Router()
import * as devolucionController from '../controllers/devolucion.controller'
...
router.post('/',
  [
    check('motivo_devolucion', 'El motivo de devolucion es obligatorio').not().isEmpty(),
    check('fecha_asignacion', 'La fecha de asignacion del activo es obligatoria').not().isEmpty(),
    check('id_activo', 'El activo es obligatorio').not().isEmpty(),
    check('id_persona', 'El empleado es obligatorio').not().isEmpty(),
    authJwt.verifyToken,
