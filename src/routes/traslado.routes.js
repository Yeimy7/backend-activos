...
const router = Router()
import * as trasladoController from '../controllers/traslado.controller.js'
...
router.post('/',
  [
    check('motivo_traslado', 'El motivo de traslado es obligatorio').not().isEmpty(),
    check('fecha_ocupacion_anterior', 'La fecha de asignacion de ambiente anterior es obligatoria').not().isEmpty(),
    check('id_activo', 'El activo es obligatorio').not().isEmpty(),
    check('id_ambiente', 'El ambiente es obligatorio').not().isEmpty(),
    authJwt.verifyToken,
